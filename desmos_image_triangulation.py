...
def desmos_write(keypoints, faces, materials):
    print('s = Calc.getState()')

    for i, face in enumerate(faces):
        pts = keypoints[face.verts][:, :2]
        print('s.expressions.list[%d] = {id: %d, type: "expression", latex: "%s", color: "%s"}' % (
            i, i,
            "\\\\operatorname{polygon}([(%f, %f),(%f, %f),(%f, %f)])" % tuple(pts.flatten()),
            '#%02x%02x%02x' % tuple((materials[face.material] * 255).astype(np.int))
        ))

    print('Calc.setState(s)')

# print('saving')
# Save the files
desmos_write(kpts3d, faces, materials)
